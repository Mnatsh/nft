...
export class NftCollection implements Contract {
    ...
    async sendGetCollection(provider: ContractProvider, getter: string) {
        //if () 
        const { stack } = await provider.get(getter, 
          getter == "get_nft_content" ?
          [{ type: "int", value: 0n }, { type: "cell", cell: beginCell().storeStringTail("hello").endCell() },] : []);
        return stack;
    }

    async sendChangeParamCollection(provider: ContractProvider, via: Sender, messageBody: Cell) {
        const messageBody1 = beginCell()
          .storeUint(1, 32) // op (op #1 = increment)
          .storeUint(0, 64) // query id
          .endCell();
        await provider.internal(via, {
          value: "0.04", // send 0.05 TON for gas
          body: messageBody
        });
      }
